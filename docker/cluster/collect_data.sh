#!/bin/bash
#SBATCH --job-name=univtac          # overridden at submission time by --job-name
...
# ── Defaults (fallback if not exported by launcher) ──────────────────────────
...
srun singularity exec --nv \
    ...
    bash -lc "
        ...
        ${TASK_NAME} ${TASK_MODE} \
        --start_seed ${START_SEED} --max_seed ${MAX_SEED} --episode_num 10
    "
